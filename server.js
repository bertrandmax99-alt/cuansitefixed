...
async function initDb() {
  ...
  if (!get('SELECT id FROM hero WHERE id=1')) {
    run(`INSERT INTO hero (id, badge, title_1, title_2, title_3, subtitle, btn_primary, btn_secondary,
      ...
      VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      ['Dipercaya UMKM & Brand Lokal \u2022 Website Siap Pakai', 'Kami Bangun', 'Website yang', 'Hasilkan Cuan',
  ...
  const currentHero = get('SELECT badge FROM hero WHERE id=1');
  if (currentHero && (currentHero.badge || '').toString().trim().startsWith('Tersedia untuk Proyek Baru')) {
    run('UPDATE hero SET badge=? WHERE id=1', ['Dipercaya UMKM & Brand Lokal • Website Siap Pakai']);
  }
...
app.get('/api/content', (req, res) => {
  ...
  const testimonials = all('SELECT * FROM testimonials ORDER BY sort_order ASC').map((t, idx) => {
    const name = (t.name || '').toString().trim();
    const quote = (t.quote || '').toString().trim();
    const role = (t.role || '').toString().trim();
    const avatarLetterRaw = (t.avatar_letter || '').toString().trim();
    const avatar_letter = avatarLetterRaw || (name ? name.charAt(0).toUpperCase() : '?');
    return {
      ...t,
